# 🏗️ 시스템 아키텍처

> 노드 구성, 데이터 흐름, MQTT 토픽 구조

> 🖥️ 서버 측 아키텍처는 [slam_mqtt_server](https://github.com/ky51301130-jpg/slam_mqtt_server) 저장소를 참조하세요.

> 🤖 이 프로젝트는 [pinklab-art/pinky_pro](https://github.com/pinklab-art/pinky_pro) ROS2 패키지를 기반으로 개발되었습니다.

---

## 📊 노드 구성도

### SLAM 모드 (slam_exploration.launch.py)

```
┌─────────────────────────────────────────────────────────────────┐
│                    SLAM 모드                                     │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌─────────────┐   ┌─────────────┐   ┌─────────────┐           │
│  │ auto_drive  │──▶│   /scan     │──▶│ map_saver   │           │
│  │   _node     │   │  /odom      │   │   _node     │           │
│  └─────────────┘   └─────────────┘   └─────────────┘           │
│        │                                    │                   │
│        ▼                                    ▼                   │
│  ┌─────────────┐                     ┌─────────────┐           │
│  │  /cmd_vel   │                     │ 서버 업로드  │           │
│  └─────────────┘                     └─────────────┘           │
│                                                                 │
│  ┌─────────────┐   ┌─────────────┐   ┌─────────────┐           │
│  │   status    │   │ mqtt_bridge │   │ collision   │           │
│  │ _display    │◀──│   _node     │◀──│ _photo_node │           │
│  │ (LED+LCD)   │   │             │   │             │           │
│  └─────────────┘   └─────────────┘   └─────────────┘           │
│        │                  ▲                                     │
│        │ Lux 기반 LED     │ MCU (mcu/sensors)                   │
│        ▼                  │                                     │
│  ┌─────────────┐          │                                     │
│  │  WS281x LED │          │                                     │
│  │  + LCD 표시 │          │                                     │
│  └─────────────┘          │                                     │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### Nav2 모드 (nav2_mode.launch.py)

```
┌─────────────────────────────────────────────────────────────────┐
│                    Nav2 모드                                     │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │              Nav2 bringup_launch.py                      │   │
│  │  ┌────────┐ ┌─────────┐ ┌──────────┐ ┌────────────┐     │   │
│  │  │  AMCL  │ │ Planner │ │Controller│ │BT Navigator│     │   │
│  │  └────────┘ └─────────┘ └──────────┘ └────────────┘     │   │
│  └─────────────────────────────────────────────────────────┘   │
│        ▲                                                        │
│        │ navigate_to_pose                                       │
│  ┌─────────────┐   ┌─────────────┐   ┌─────────────┐           │
│  │ nav2_goal   │◀──│ MQTT/PLC    │   │ camera      │           │
│  │   _node     │   │   명령      │   │ _stream_node│           │
│  └──────┬──────┘   └─────────────┘   └─────────────┘           │
│        │ 도착 시 dock_enable                                      │
│        ▼                                                        │
│  ┌─────────────┐   ┌─────────────┐   ┌─────────────┐           │
│  │ aruco_dock  │   │   status    │◀──│ mqtt_bridge │           │
│  │   _node     │   │  _display   │   │   _node     │           │
│  │ (정밀 도킹) │   │ (LED+LCD)   │   └─────────────┘           │
│  └─────────────┘   └─────────────┘          ▲                  │
│        │                                 │ MCU (Lux)          │
│        ▼ ArUco 마커 감지                   │                    │
│  ┌─────────────┐                          │                    │
│  │   카메라    │                          │                    │
│  │  PORT_A/B  │                          │                    │
│  └─────────────┘                          │                    │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 📡 MQTT 토픽 구조

### 전체 토픽 맵

```
MQTT Broker (192.168.0.3:1883)
│
├── 📥 외부 → 로봇 (수신)
│   │
│   ├── mcu/
│   │   └── sensors               # MCU 센서 데이터
│   │
│   ├── plc/
│   │   ├── location              # 위치 이름 이동
│   │   └── goal                  # 좌표 이동
│   │
│   ├── server/
│   │   └── locations             # 위치 프리셋 업데이트
│   │
│   └── nav2/
│       └── initial_pose          # AMCL 초기 위치 설정
│
├── 📤 로봇 → 외부 (발행)
│   │
│   ├── robot/
│   │   ├── navigate_to_pose      # Goal 요청 (모니터링용)
│   │   ├── nav_status            # 진행 상태
│   │   ├── nav_result            # 완료/실패 결과
│   │   └── arrived               # 목표 도착
│   │
│   ├── slam_mode                 # SLAM/NAV2/IDLE
│   │
│   ├── battery/
│   │   └── status                # 배터리 전압 (옵션)
│   │
│   ├── collision/
│   │   └── photo_ready           # 충돌 사진 알림
│   │
│   ├── qr_detected               # QR 코드 감지
│   │
│   └── ros/
│       └── map_cycle_complete    # 맵 사이클 완료
│
└── 📊 AI 서버 발행
    └── mqtt/pinky/
        ├── detection             # YOLO 감지 결과
        └── obstacle_type         # 장애물 유형
```

---

## 📬 MQTT 메시지 형식 (JSON)

### 로봇 → 서버 (발행)

```json
// robot/navigate_to_pose - Goal 요청
{
  "type": "location",
  "value": "station1",
  "timestamp": 1733567890.123
}

// robot/nav_status - 진행 상태
{
  "status": "NAVIGATING",
  "message": "(1.50, 2.00)",
  "goal": {"x": 1.5, "y": 2.0, "yaw": 0.0}
}

// robot/nav_result - 완료/실패 결과
{
  "result": "SUCCEEDED",
  "message": "Goal reached!",
  "goal": {"x": 1.0, "y": 0.5, "yaw": 0},
  "timestamp": 1733567900.456
}

// robot/arrived - 목표 도착
{
  "arrived": true,
  "timestamp": 1733567901.789
}

// slam_mode - 로봇 모드
{
  "mode": "SLAM",
  "timestamp": 1733567800.000
}

// ros/map_cycle_complete - 맵 저장 완료
{
  "cycle": 8,
  "pgm_path": "/home/pinky/saved_maps/map_07.pgm",
  "yaml_path": "/home/pinky/saved_maps/map_07.yaml",
  "timestamp": 1733568000.000
}

// collision/photo_ready - 충돌 사진
{
  "filename": "collision_20251207_123456_0.25m.jpg",
  "url": "http://192.168.0.5:5001/photos/collision_20251207_123456_0.25m.jpg",
  "distance": 0.25,
  "timestamp": 1733568100.000
}

// qr_detected - QR 감지
{
  "data": "STATION_A",
  "timestamp": 1733568200.000
}

// battery/status - 배터리
{
  "voltage": 11.8,
  "timestamp": 1733567860.000
}
```

### 외부 → 로봇 (수신)

```json
// mcu/sensors - MCU 센서
{
  "Lux": 150.5,
  "Temperature": 25.3,
  "Humidity": 45.0
}

// plc/location - 위치 이름
"station1"

// plc/goal - 좌표 Goal
{
  "x": 2.0,
  "y": 1.5,
  "yaw": 1.57
}

// server/locations - 위치 프리셋
{
  "station1": {"x": 1.0, "y": 0.5, "yaw": 0},
  "station2": {"x": 2.0, "y": 1.0, "yaw": 1.57},
  "charging": {"x": -0.5, "y": 0, "yaw": 3.14}
}

// nav2/initial_pose - 초기 위치
{
  "x": 0.0,
  "y": 0.0,
  "yaw": 0.0
}
```

### AI 서버 발행

```json
// mqtt/pinky/detection - YOLO 감지
{
  "objects": [
    {"class": "person", "confidence": 0.92, "bbox": [100, 50, 200, 300]},
    {"class": "box", "confidence": 0.87, "bbox": [250, 100, 350, 200]}
  ],
  "timestamp": 1733568300.000
}

// mqtt/pinky/obstacle_type - 장애물 유형
{
  "type": "person",
  "action": "wait",
  "timestamp": 1733568301.000
}
```

---

## 📝 ROS2 ↔ MQTT 매핑 테이블

| ROS2 Topic | MQTT Topic | 방향 |
|------------|------------|------|
| `/robot_mode` | `slam_mode` | ROS2 → MQTT |
| `/nav2/status` | `robot/nav_status` | ROS2 → MQTT |
| `/nav2/status` (결과) | `robot/nav_result` | ROS2 → MQTT |
| `/nav2/arrived` | `robot/arrived` | ROS2 → MQTT |
| `/collision/photo_ready` | `collision/photo_ready` | ROS2 → MQTT |
| `/qr_detected` | `qr_detected` | ROS2 → MQTT |
| `/battery/voltage` | `battery/status` | ROS2 → MQTT |
| `/map_saver/cycle_complete` | `ros/map_cycle_complete` | ROS2 → MQTT |
| `mcu/sensors` | `/mqtt/mcu_sensors` | MQTT → ROS2 |
| `plc/location` | `/mqtt/plc_location` | MQTT → ROS2 |
| `plc/goal` | `/mqtt/plc_goal` | MQTT → ROS2 |

---

## 🔄 데이터 흐름

### Nav2 Goal 흐름

```
PLC/서버                    로봇                         Nav2
    │                        │                           │
    │  plc/location          │                           │
    ├───────────────────────▶│ mqtt_bridge_node          │
    │                        │       │                   │
    │                        │       ▼                   │
    │  robot/navigate_to_pose│ nav2_goal_node            │
    │◀───────────────────────│       │                   │
    │                        │       │ NavigateToPose    │
    │                        │       ├──────────────────▶│
    │                        │       │                   │
    │  robot/nav_status      │       │◀── feedback ──────│
    │◀───────────────────────│       │                   │
    │                        │       │                   │
    │  robot/nav_result      │       │◀── result ────────│
    │◀───────────────────────│       │                   │
    │                        │       │                   │
```

### MCU 센서 → LED 제어 흐름

```
MCU (ESP32)              MQTT Broker              ROS2
    │                        │                      │
    │  {"Lux": 150.5}        │                      │
    ├──── mcu/sensors ──────▶│                      │
    │                        │                      │
    │                        │  /mqtt/mcu_sensors   │
    │                        ├─────────────────────▶│ mqtt_bridge_node
    │                        │                      │       │
    │                        │                      │       ▼
    │                        │                      │ led_controller_node
    │                        │                      │   (Lux 기반 LED 제어)
```
